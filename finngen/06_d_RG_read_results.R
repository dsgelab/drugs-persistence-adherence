...
get_rg <- function(x)
{
  ...
  tt <- system( paste0('grep "^ /home/ivm/drugs/results/R10_GWAS/persistence/munged/" ', x), intern = T)
...
phenolist <- c("statins", "blood_pressure", "breast_cancer", "clopi_dipy", "doac")
...
for (p in phenolist) {
  temp <- get_rg(paste0('/home/ivm/drugs/results/R10_GWAS/persistence/rg/',p,'.sumstats.gz_RGS.log'))
  d <- rbind(d,temp)
  
  temp <- get_h2(paste0('/home/ivm/drugs/results/R10_GWAS/persistence/h2/',p,'.sumstats.gz_H2.log'))
...
d$p1 <- sub('/home/ivm/drugs/results/R10_GWAS/persistence/munged/','',d$p1)
d$p1 <- sub('.sumstats.gz','',d$p1)
d$p2 <- sub('/home/ivm/drugs/data/genetic_correlations/munged_sumstats/','',d$p2)
d$p2 <- sub('.sumstats.gz','',d$p2)
cols.num <- seq(3,12)
d[cols.num] <- sapply(d[cols.num],as.numeric)

system('mkdir /home/ivm/drugs/results/manuscript/rgs/')
fwrite(d, '/home/ivm/drugs/results/manuscript/rgs/rgs_persistence.tsv', sep = "\t", quote = F, na = "NA")
fwrite(h2, '/home/ivm/drugs/results/manuscript/rgs/h2_persistence.tsv', sep = "\t", quote = F, na = "NA")
