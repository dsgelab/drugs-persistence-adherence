...
task combine {
    ...
    command <<<
        ...
        if "${traitType}" == "quantitative":
            mapping = OrderedDict([
                ...
                ("info", ("imputationInfo", [(float, ()), (str.format, ("{:.2e}"))])),
                ("n", ("N", [(float, ()), (int, ())]))
            ])
        else:
            mapping = OrderedDict([
        ...
        echo "`date` filtering AF 0.001, INFO 0.6"
        gunzip -c ${prefix}${pheno}.gz | awk '
        NR==1 {for (i=1;i<=NF;i++) a[$i]=i; print $0}
        NR>1 && $a["af_alt"] >= 0.001 && $a["info"] >= 0.6
        ' | bgzip > ${prefix}${pheno}.af_0.001.info_0.6.txt.gz
        echo "`date` tabixing"
        tabix -S 1 -b 2 -e 2 -s 1 ${prefix}${pheno}.af_0.001.info_0.6.txt.gz
        echo "`date` plotting qq and manha filtered"
        qqplot.R --file ${prefix}${pheno}.af_0.001.info_0.6.txt.gz --bp_col "${bp_col}" --pval_col "${p_valcol}" --chrcol "${chrcol}" --loglog_pval ${loglog_pval}
        echo "`date` done"
    >>>
    ...
    output {
        ...
        File out_filt = prefix + pheno + ".af_0.001.info_0.6.txt.gz"
        File out_filt_ind = prefix + pheno + ".af_0.001.info_0.6.txt.gz.tbi"
