...
get_rg <- function(x)
{
  ...
  tt <- system( paste0('grep "^ukbb" ', x), intern = T)
...
d <- NULL
for (p in phenolist) {
  temp <- get_rg(paste0('/home/cordioli/drugs/results/20210520_UKB/rg/',p,'_RGS.log'))
  d <- rbind(d,temp)
  
  temp <- get_h2(paste0('/home/cordioli/drugs/results/20210520_UKB/h2/',p,'_H2.log'))
...
d$p1 <- sub('/home/cordioli/drugs/results/20210520_UKB/','',d$p1)
...
dd$p1[dd$p1=="ukbb_statins.AF0.001.gz.sumstats.gz"] <- "Statins"
...
pdf("/home/cordioli/drugs/results/20210520_UKB/RGS.pdf", width = 9, height = 8)
