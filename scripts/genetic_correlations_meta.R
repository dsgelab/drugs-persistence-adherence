...
get_rg <- function(x)
{
  ...
  tt <- system( paste0('grep "^meta" ', x), intern = T)
...
phenolist <- c('meta_statins')

h2 <- NULL
d <- NULL
for (p in phenolist) {
  temp <- get_rg(paste0('/home/cordioli/drugs/results/20210522_meta/',p,'_RGS.log'))
  d <- rbind(d,temp)
  
  temp <- get_h2(paste0('/home/cordioli/drugs/results/20210522_meta/',p,'_H2.log'))
...
d$p1 <- sub('/home/cordioli/drugs/results/20210522_meta/','',d$p1)
...
to.keep <- c("ADHD_2017.LDSC.sumstats.gz", "BMI.LDSC.sumstats.gz", "CAD.LDSC.sumstats.gz", "EA.LDSC.sumstats.gz", "IBD.LDSC.sumstats.gz",
             ...
             "Xue_et_al_T2D_META_Nat_Commun_2018.sumstats.gz")
             
             #, "BOLT_sex_combined_s340951_unrelated_WhiteBritish.LDL.sumstats.gz")
...
dd$p1[dd$p1=="meta_statins.sumstats.gz"] <- "Statins"
...
pdf("/home/cordioli/drugs/results/20210522_meta/RGS.pdf", width = 10, height = 8)
...
png("/home/cordioli/drugs/results/20210522_meta/RGS.png", width = 9, height = 7, units = "in", res = 300)
ggplot(ddd, aes(x=rg, y=p2, colour = factor(sig))) +
  geom_point() +
  geom_errorbarh(aes(xmin=rg-1.96*se, xmax=rg+1.96*se), height=0.1) +
  # geom_text(aes(label=p_label), color = "red", size = 5) +
