...
for (p in phenolist) {
  temp <- get_rg(paste0('/home/ivm/drugs/results/R8_20220112/rg_adh_adh/',p,'.af_0.01.info_0.6.gz.rsid.gz.sumstats.gz_RGS.log'))
  d <- rbind(d,temp)
}

d$p1 <- sub('/home/ivm/drugs/results/R8_20220112/munged/','',d$p1)
d$p2 <- sub('/home/ivm/drugs/results/R8_20220112/munged/','',d$p2)
cols.num <- seq(3,12)
...
ddd <- dd %>% 
  rowwise() %>%
  mutate(pair = sort(c(p1, p2)) %>% paste(collapse = ",")) %>%
  group_by(pair) %>%
  distinct(pair, .keep_all = T) %>% 
  mutate(label = paste0("rg: ", rg, "\np: ", p)) %>% 
  separate(pair, c("med1","med2"), sep = ",")

library(ggplot2)
ggplot(ddd, aes(x = p1, y = p2, fill = rg)) +
  geom_raster() +
...
png("/home/ivm/drugs/results/R8_20220112/RGS_adh_adh_R8.png", width = 14, height = 11, units = "in", res = 300)
