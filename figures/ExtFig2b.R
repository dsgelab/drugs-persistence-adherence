...
setwd('/home/ivm/drugs/results/R10_GWAS/adherence/sumstats/')
...
for (s in stats){
  ...
  drug_names <- data.frame(d = c("blood_pressure", "breast_canc", "clopi_dipy", "doac", "statins"),
                           ...
                           N = c("126,126", "9,760", "12,618", "2,491", "111,309"))
  ...
  tmp <- tmp1 %>%
    ...
    mutate(chrom=gsub('X', '23', `#chrom`),
           ...
           drug_n=paste0(drug_names$name[drug_names$d == d], " - N: ",drug_names$N[drug_names$d == d])) %>% 
...
png("/home/ivm/drugs/results/manuscript/R10_manhattan_adherence.png", width=1600, height=1800, res = 300)
