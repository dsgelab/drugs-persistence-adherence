...
pgs <- fread("/Users/cordioli/Projects/drugs/manuscript/pgs/pgs_results/R10_betas_prs_persistence.tsv")
...
pgs <- pgs %>% 
  ...
  mutate(OR = exp(estimate),
         lower = exp(estimate - (multiplier * std.error)),
         upper = exp(estimate + (multiplier * std.error)) )
...
fwrite(pgs, 'Projects/drugs/manuscript/pgs/pgs_per_final.tsv', sep = "\t")
...
pgs <- pgs %>% filter(drug != "Antiplatelets")

png("/Users/cordioli/Projects/drugs/manuscript/figures/ExtFig3.png", width = 12, height = 8, units = "in", res = 300)
ggplot(pgs) +
  facet_grid(factor(class) ~ factor(drug), scales = "free_y", space = "free_y") +
  geom_pointrange(aes(y=trait,x=OR,xmin=lower, xmax=upper, color = factor(fdr_significant)), size = .12) +
  theme_minimal() +
